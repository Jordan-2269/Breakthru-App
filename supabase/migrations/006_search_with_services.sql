-- Replace search function to also return each listing's service type IDs
-- so the client can compute accurate match scores against a child's needs
DROP FUNCTION IF EXISTS search_listings_near(double precision, double precision, double precision, uuid[]);
...
RETURNS TABLE (
  ...
  WHERE
    ...
    AND (
      p_service_type_ids IS NULL
      OR EXISTS (
        SELECT 1 FROM public.business_services bs
        WHERE bs.listing_id = bl.id
          AND bs.service_type_id = ANY(p_service_type_ids)
      )
    )
